...
if ( ! function_exists( 'vikata_get_setting' ) ) {
	/**
	 * A wrapper function to get our settings.
	 *
	 */
	function vikata_get_setting( $setting ) {
		$vikata_settings = wp_parse_args(
			get_option( 'vikata_settings', array() ),
			vikata_get_defaults()
		);

		return $vikata_settings[ $setting ];
	}
}

if ( ! function_exists( 'vikata_get_layout' ) ) {
	/**
	...
	function vikata_get_layout() {
		...
		$layout = $vikata_settings['layout_setting'];

		// Get the individual page/post sidebar metabox value
		$layout_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_vikata-sidebar-layout-meta', true ) : '';
		...
		if ( is_single() && ! $buddypress ) {
			$layout = null;
			$layout = $vikata_settings['single_layout_setting'];
		}
		...
		if ( is_home() || is_archive() || is_search() || is_tax() ) {
			$layout = null;
			$layout = $vikata_settings['blog_layout_setting'];
		}

		// Finally, return the layout
		return apply_filters( 'vikata_sidebar_layout', $layout );
	}
}

if ( ! function_exists( 'vikata_get_footer_widgets' ) ) {
	/**
	 ...
	 */
	function vikata_get_footer_widgets() {
		// Get current post
		global $post;
		...
		$widgets = $vikata_settings['footer_widget_setting'];

		// Get the individual footer widget metabox value
		$widgets_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_vikata-footer-widget-meta', true ) : '';
		...
		// Finally, return the layout
		return apply_filters( 'vikata_footer_widgets', $widgets );
	}
}

if ( ! function_exists( 'vikata_show_excerpt' ) ) {
	/**
	 * Figure out if we should show the blog excerpts or full posts
	 *
	 */
	function vikata_show_excerpt() {
		// Get current post
		global $post;
		...
		$more_tag = apply_filters( 'vikata_more_tag', strpos( $post->post_content, '<!--more-->' ) );

		// Check the post format
		$format = ( false !== get_post_format() ) ? get_post_format() : 'standard';

		// Get the excerpt setting from the Customizer
		$show_excerpt = ( 'excerpt' == $vikata_settings['post_content'] ) ? true : false;
		...
		// Return our value
		return apply_filters( 'vikata_show_excerpt', $show_excerpt );
	}
}

if ( ! function_exists( 'vikata_show_title' ) ) {
	/**
	 ...
	 */
	function vikata_show_title() {
		return apply_filters( 'vikata_show_title', true );
	}
}

if ( ! function_exists( 'vikata_padding_css' ) ) {
	/**
	 ...
	 */
	function vikata_padding_css( $top, $right, $bottom, $left ) {
...
if ( ! function_exists( 'vikata_get_link_url' ) ) {
	/**
	 * Return the post URL.
	 *
...
if ( ! function_exists( 'vikata_get_navigation_location' ) ) {
	/**
	 ...
	 */
	function vikata_get_navigation_location() {
		return apply_filters( 'vikata_navigation_location', vikata_get_setting( 'nav_position_setting' ) );
	}
}

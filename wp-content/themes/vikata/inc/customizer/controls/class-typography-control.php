...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Vikata_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Vikata_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'vikata-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'vikata-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), VIKATA_VERSION, true );
			wp_enqueue_style( 'vikata-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), VIKATA_VERSION );

			wp_enqueue_script( 'vikata-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'vikata-typography-selectWoo' ), VIKATA_VERSION, true );
			wp_enqueue_style( 'vikata-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), VIKATA_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'vikata_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'vikata' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'vikata' );
			$this->json[ 'google_fonts' ] = apply_filters( 'vikata_typography_customize_list', vikata_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = vikata_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'vikata' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'vikata' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'vikata' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'vikata' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="vikata-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="vikata-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="vikata-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="vikata-font-transform">
					<label>

...
if ( ! function_exists( 'vikata_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'vikata_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function vikata_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', vikata_typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $vikata_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $vikata_settings[$key], ':' ) !== false ) {
					$vikata_settings[$key] = current( explode( ':', $vikata_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $vikata_settings[$key] );

				// Grab the variants using the plain name
				$variants = vikata_get_google_font_variants( $vikata_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'vikata_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'vikata_fonts_subset','' );
		...
		$default_fonts = VIKATA_DEFAULT_FONTS;

		// Create our URL using the arguments
		$fonts_url = add_query_arg( $font_args, '//fonts.googleapis.com/css' );

		// Enqueue our fonts
		wp_enqueue_style('vikata-default-fonts', $default_fonts, array(), null, 'all' );
		if ( $google_fonts ) {
			wp_enqueue_style('vikata-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'vikata_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'vikata_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function vikata_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'vikata_fonts_customize_register' ) ) {
			// Bail if Vikata Premium is active
			return;
		}
		...
		$defaults = vikata_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Vikata_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'vikata_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'vikata_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'vikata_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new Vikata_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'vikata' ),
					'settings' => array(
						'family' => 'vikata_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'vikata_settings[body_font_weight]',
						'transform' => 'vikata_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_setting(
			'vikata_settings[body_font_size]',
			array(
		...
		$wp_customize->add_control(
			new Vikata_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'vikata' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'vikata_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'vikata_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'vikata_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new Vikata_Range_Slider_Control(
				...
				array(
					'type' => 'vikata-range-slider',
					'description' => __( 'Line height', 'vikata' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'vikata_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'vikata_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new Vikata_Range_Slider_Control(
				...
				array(
					'type' => 'vikata-range-slider',
					'description' => __( 'Paragraph margin', 'vikata' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'vikata_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'vikata_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function vikata_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'vikata_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'vikata_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'vikata_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_vikata_get_all_google_fonts_ajax', 'vikata_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function vikata_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'vikata_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'vikata_customize_nonce' ] ), 'vikata_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'vikata_customize_nonce', 'vikata_customize_nonce' );
		...
		$fonts = vikata_get_all_google_fonts();
...
if ( ! function_exists( 'vikata_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: vikata_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function vikata_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, vikata_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'vikata_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'vikata_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: vikata_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function vikata_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'vikata_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function vikata_get_font_family_css( $font, $settings, $default ) {
		$vikata_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'vikata_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $vikata_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'vikata_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . vikata_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'vikata_add_to_font_customizer_list' ) ) {
	add_filter( 'vikata_typography_customize_list', 'vikata_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function vikata_add_to_font_customizer_list( $fonts ) {
		...
		$all_fonts = vikata_get_all_google_fonts();
		$select_fonts = vikata_get_all_google_fonts( apply_filters( 'vikata_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $vikata_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $vikata_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $vikata_settings[ $setting ], vikata_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $vikata_settings[ $setting ] ) ) ] = array(
				'name' => $vikata_settings[ $setting ],
		...
		if ( function_exists( 'vikata_secondary_nav_get_defaults' ) ) {
			...
			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], vikata_typography_default_fonts() ) ) {

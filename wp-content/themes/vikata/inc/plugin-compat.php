...
add_action( 'after_setup_theme', 'vikata_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function vikata_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'vikata_construct_sidebars' );
}

if ( ! function_exists( 'vikata_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'vikata_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function vikata_woocommerce_start() { ?>
...
if ( ! function_exists( 'vikata_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'vikata_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function vikata_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'vikata_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'vikata_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function vikata_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'vikata_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'vikata_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'vikata_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function vikata_bbpress_css() {
...
if ( ! function_exists( 'vikata_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'vikata_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function vikata_buddypress_css() {
...
if ( ! function_exists( 'vikata_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'vikata_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function vikata_beaver_builder_css() {

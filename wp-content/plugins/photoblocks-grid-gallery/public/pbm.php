...
ini_set('gd.jpeg_ignore_warning', true);																// Version of this script 
...
if(! defined('DEBUG_ON') )					define ('DEBUG_ON', isset($_GET["debug"]));								// Enable debug logging to web server error log (STDERR)
...
if(! defined('CURL_TIMEOUT') )				define ('CURL_TIMEOUT', 30);							// Timeout duration for Curl. This only applies if you have Curl installed and aren't using PHP's default URL fetching mechanism.
...
class timthumb {
	...
	protected function serveErrors(){
		...
		echo '<br />TimThumb version : PhotoBlocks</pre>';
	}
	...
	protected function processImageAndWriteToCache($localImage){
		...
		$max_size = (int) abs ($this->param('max', 0));
		...
		if ($max_size == 0 && $new_width == 0 && $new_height == 0) {
